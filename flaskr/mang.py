...
def searching():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            if info is not None:
                cursor.execute("select p.id,p.title,p.body,p.created,p.author_id,u.username from post p,imp_user u where p.id=%s",(info,))
                posts=cursor.fetchall()
                db.commit()
...
@bp.route('/<pid>/add_hashtag',methods=('GET','POST'))
def add_hashtag(pid):
    db = get_db()
    cursor = db.cursor()
    if request.method == 'POST':
        addtag = request.form.get('addtag')
        if addtag is not None:
            cursor.execute("select id from tag where name=(%s)",(addtag,))
            info=cursor.fetchone()
            cursor.execute("insert into tags_post(notes,tag) values(%s,%s) ",(pid,info[0]))
            db.commit()
            return redirect(url_for('mang.details',pid=pid))   
    else:
